...
namespace ConflictResolution
{
	public partial class Form1 : Form
	{
		...
		private async void GetConflictsButton_Click(object sender, EventArgs e)
		{
			...
			var iterator = this._container.Conflicts.GetConflictQueryIterator<ConflictProperties>();
			while (iterator.HasMoreResults)
			{
				var conflicts = await iterator.ReadNextAsync();
				foreach (var conflict in conflicts)
				{
					this.ConflictsListBox.Items.Add(conflict.Id);
					this._conflicts.Add(conflict);
				}
			}
			this.GetConflictsButton.Text = $"Get Conflicts ({this._conflicts.Count})";
			this.Enabled = true;
		}
