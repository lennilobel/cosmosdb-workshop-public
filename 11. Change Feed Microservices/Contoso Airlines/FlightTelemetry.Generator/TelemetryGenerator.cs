...
namespace FlightTelementry.Generator
{
	public class TelemetryGenerator
	{
		...
		private readonly FlightRepo _repo = new();
		private readonly FlightSpatial _spatial = new();
		...
		public async Task ResetDemos()
		{
			if (!this.Confirm($"This will reset the {Constants.DatabaseName} database for a fresh demo, without discarding the location container."))
			{
				return;
			}

			await this.CreateContainers(exceptLocationContainer: true);
			await this.CreateAirports();
			await this.CreateFlights();
		}
		...
		private async Task CreateContainers(bool exceptLocationContainer = false)
		{
			Console.WriteLine("Creating containers");
			foreach (var container in this.Containers)
			{
				if (exceptLocationContainer && container.ContainerName == Constants.LocationContainerName)
				{
					continue;
				}
				...
				try
				{
					try
					{
						await Cosmos.Client.GetDatabase(Constants.DatabaseName).GetContainer(container.ContainerName).DeleteContainerAsync();
					}
				...
				catch (Exception ex)
				{
					...
					Console.WriteLine($"Error creating container '{container.ContainerName}': {ex.Message}");
					Console.ResetColor();
				}
			}
		...
		private async Task CreateContainer(string containerName, string partitionKey, int? throughput, bool autoScale, int? timeToLive)
		{
			...
			{
				throughputProperties = ThroughputProperties.CreateAutoscaleThroughput(throughput.Value * 10);
		...
		{
			var airports = await this._repo.GetAirports();
			var flights = await this._repo.GetFlights();
			...
			foreach (var flight in flights)
			{
				try
				{
					await container.DeleteItemAsync<object>(flight.FlightNumber, new PartitionKey("location"));
					ctr++;
				}
				catch (CosmosException ex) when (ex.StatusCode == System.Net.HttpStatusCode.NotFound)
				{
			...
			foreach (var airport in airports)
			{
				try
				{
					await container.DeleteItemAsync<object>(airport.Code, new PartitionKey("arrival"));
					ctr++;
				}
				catch (CosmosException ex) when (ex.StatusCode == System.Net.HttpStatusCode.NotFound)
				{
			...
			foreach (var flight in flights)
			{
				var result = await this._repo.QueryLocation(flight.FlightNumber);
		...
		public async Task GenerateData(bool continuous)
		{
			...
			Console.WriteLine();
			Console.WriteLine();
			Console.WriteLine($"Flight  From > To  Distance  Duration  Remaining           Speed    Altitude   Location");
			Console.WriteLine($"------- ---------- --------  --------  ------------------  -------- ---------  -------------------");
		...
		private List<LocationEvent> GenerateTelemetry(bool continuous)
		{
			...
			foreach (var flight in this._flights)
			{
				...
				Console.WriteLine(string.Concat(
					...
					$"{flight.DistanceMiles,4} mi   ",
					$"{Math.Round(flight.DurationMinutes / 60, 2),-4} hr    ",
					$"{remainingMiles,4} mi, ",
					$"{Math.Round(remainingMinutes / 60, 2),-4:0.00} hr   ",
					$"{item.Speed} mph  ",
					$"{item.Altitude} ft ",
					$"{item.Latitude,9:##00.0000}, {item.Longitude,9:##00.0000}   "
				));
				list.Add(item);
