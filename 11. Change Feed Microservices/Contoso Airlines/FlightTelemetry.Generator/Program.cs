...
namespace FlightTelementry.Generator
{
	...
	{
		private static readonly FlightRepo _repo = new();
		private static readonly TelemetryGenerator _generator = new();
		...
		private static async Task RunOperation(string[] args)
		{
			try
			{
				...
				else if (operation.Matches("rd"))
				{
					await _generator.ResetDemos();
				}
		...
		private static void ShowUsage()
		{
			...
			Console.WriteLine("  rd            reset demo");
			Console.WriteLine("  rf            reset flights");
