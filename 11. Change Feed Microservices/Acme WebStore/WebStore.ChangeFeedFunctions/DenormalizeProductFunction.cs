	...
	class DenormalizeProductFunction
	{
		...
		public static async Task DenormalizeProduct(
			[CosmosDBTrigger(
				...
				LeaseContainerPrefix = "DenormalizeProduct"
			)]
		...
		private static async Task ProcessChange(JObject document, ILogger logger)
		{
		...
		private static async Task UpdateProductCategoryName(string categoryId, string categoryName, ILogger logger)
		{
			...
			var options = new QueryRequestOptions { PartitionKey = new PartitionKey(categoryId) };
